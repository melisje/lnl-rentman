...
use App\Models\Rentman\SerialNumber;
use App\Models\Rentman\StockLocation;
use App\Services\Rentman\Api\AbstractRentmanFetcher;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Schema;

class StockLocationFetcher extends AbstractRentmanFetcher
{
    /**
    ...
    public function processPage(string $account, array $items): void
    {
        foreach ($items as $item)
        {
            ...
            $model = StockLocation::updateOrCreate(
            ...
            foreach ($custom as $key => $value)
            {
                ...
                if (Schema::hasColumn('rm_stocklocations', $fieldName)) {
                    // De kolom bestaat, je kunt hier je logica uitvoeren
                    $model->$fieldName = $value; // Dynamisch veld toevoegen aan het model (zorg dat je deze velden ook in $fillable of $guarded hebt staan)
                }
            }
